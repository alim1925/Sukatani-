...
public class ScheduleView extends VBox {
    ...
    private TableView<ScheduleTask> createScheduleTable() {
        ...
        TableColumn<ScheduleTask, String> cropCol = new TableColumn<>("Tanaman");
        cropCol.setCellValueFactory(data -> {
            int cropId = data.getValue().getCropId();
            String cropName = cropComboBox.getItems().stream()
                    .filter(c -> c.getId() == cropId)
                    .map(Crop::getName)
                    .findFirst().orElse("ID: " + cropId);
            return new SimpleStringProperty(cropName);
        });
